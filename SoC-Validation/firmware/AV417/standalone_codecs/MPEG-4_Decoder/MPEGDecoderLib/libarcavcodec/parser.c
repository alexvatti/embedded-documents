...
int ff_combine_frame(ParseContext *pc, int next, uint8_t **buf, int *buf_size)
{
    ...
    if(next == END_NOT_FOUND){
        pc->buffer= av_fast_realloc(pc->buffer, (unsigned *)&pc->buffer_size, (*buf_size) + pc->index + FF_INPUT_BUFFER_PADDING_SIZE);
    ...
    *buf_size=
    ...
    /* append to buffer */
    if(pc->index){
        pc->buffer= av_fast_realloc(pc->buffer, (unsigned *)&pc->buffer_size, next + pc->index + FF_INPUT_BUFFER_PADDING_SIZE);
