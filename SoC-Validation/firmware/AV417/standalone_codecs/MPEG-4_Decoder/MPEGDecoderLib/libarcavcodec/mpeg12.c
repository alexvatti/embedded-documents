                          ...
                          DCTELEM block[12][64])
{
    ...
    if (s->mb_skip_run-- != 0) {
        ...
        if (s->pict_type == P_TYPE) {
            ...
            s->field_select[0][0]=s->field_select[0][1]=s->field_select[1][0] = s->field_select[1][1] = (s->picture_structure == PICT_BOTTOM_FIELD);
            #endif
...
#else
        // C_coded_vlc (&mb_ptype, 6, "mk_table", 13, "MPEG macroblock P type");
        AURORA_SET_TABLE(ARC_VLC_TABLE_MB_PTYPE);
...
#else
        // C_coded_vlc (&mb_btype, 6, "mk_table", 14, "MPEG macroblock B type");
        AURORA_SET_TABLE(ARC_VLC_TABLE_MB_BTYPE);
        mb_type = aurora_get_vlc(&s->gb);
#endif
        if (mb_type < 0){
...
#else
            // C_vlc (&mb_pat, 6, "mk_table", 11, "MPEG coded block pattern");
            AURORA_SET_TABLE(ARC_VLC_TABLE_MB_PAT);
            cbp = aurora_get_vlc(&s->gb);
#endif
...
#else
    // C_vlc (&mb_motion, 6, "mk_table", 12, "MPEG motion vector");
    AURORA_SET_TABLE(ARC_VLC_TABLE_MB_MOTION);
...
static inline int decode_dc(GetBitContext *gb, int component)
{
    ...
    AURORA_SET_TABLE((component) ? ARC_VLC_TABLE_DC_CHROMA : ARC_VLC_TABLE_DC_LUM);
    code = aurora_get_vlc(gb);
...
#if defined(ARC_BITSTREAM_READER) && defined(AURORA_DEQUANT)
...
//    const uint16_t *quant_matrix= s->intra_matrix;
    const uint16_t *quant_matrix= s->intra_matrix_sdm;
    ...
    {
        OPEN_READER(re, &s->gb);
        AURORA_SET_TABLE(ARC_VLC_TABLE_RL_MPEG1);
        /* now quantify & encode AC coefs */
        for(;;) {
            ...
            else if(level != 0) 
            {
                i += run;
                j = scantable[i];
//rk                level= (level*qscale*quant_matrix[j])>>4;
//rk                level= (level-1)|1;
...
*/
    ...
    mpeg1_dequant_intra_simd( (unsigned int)(block) - (unsigned int)SDM_Bytes,
                    (unsigned int)(quant_matrix) - (unsigned int)SDM_Bytes,
                    (int)block[0], qscale);
   return 0;
}
...
//        quant_matrix = s->intra_matrix;
        quant_matrix = s->intra_matrix_sdm;
        component = 0;
    }else{
//        quant_matrix = s->chroma_intra_matrix;
        quant_matrix = s->chroma_intra_matrix_sdm;
        component = (n&1) + 1;
    }
    diff = decode_dc(&s->gb, component);
    if (diff >= 0xffff)
...
//    mismatch = block[0] ^ 1;
    ...
    AURORA_SET_TABLE(s->intra_vlc_format ? ARC_VLC_TABLE_RL_MPEG2 : ARC_VLC_TABLE_RL_MPEG1);
...
//    block[63]^= mismatch&1;
    ...
    mpeg2_dequant_intra_simd( (unsigned int)(block) - (unsigned int)SDM_Bytes,
                    (unsigned int)(quant_matrix) - (unsigned int)SDM_Bytes,
                    (int)block[0], qscale);
...
#else //~(defined(ARC_BITSTREAM_READER) && defined(AURORA_DEQUANT))
...
#ifdef MPEG2_ISO_COMPLIANCE 
            /* saturation */
              block[j] = (level>2047) ?  2047 :  (level<-2048) ? -2048 : level; /* positive */
#else
              block[j] = level;
#endif
        }
                               ...
                               int n)
{
...

    {
        ...
        if (v & 2) {
            ...
            #ifdef MPEG2_ISO_COMPLIANCE 
             level = (level>2047) ?  2047 :  (level<-2048) ? -2048 : level; /* positive */
            #endif
            block[0] = level;
            mismatch ^= level;
            i++;
            ...
            if(s->aspect_ratio_info > 1)
            {
                if( (s1->pan_scan.width == 0 )||(s1->pan_scan.height == 0) )
                {
                    ...
                    s->avctx->sample_aspect_ratio=
                        av_div_q(
                         mpeg2_aspect[s->aspect_ratio_info], temp
                         );
                }
                else
                {
                    ...
                    s->avctx->sample_aspect_ratio=
                        av_div_q( mpeg2_aspect[s->aspect_ratio_info], temp );
                }
            }
            else
            {
                             ...
                             const uint8_t **buf, int buf_size)
{
    ...
    AURORA_SET_TABLE(ARC_VLC_TABLE_MB_ADDR_INC);
    for(;;)
    {
#ifndef ARC_BITSTREAM_READER
        int code = get_vlc2(&s->gb, mbincr_vlc.table, MBINCR_VLC_BITS, 2);
#else
        // C_vlc (&mb_addr_inc, 6, "mk_table", 8, "MPEG macroblock address increment");
        int code = aurora_get_vlc(&s->gb);
#endif
        if (code < 0){
    ...
    //store de-quant matrix in SDM
    memcpy(s->intra_matrix_sdm, s->intra_matrix, (2*8*8));
    memcpy(s->inter_matrix_sdm, s->inter_matrix, (2*8*8));

    memcpy(s->chroma_intra_matrix_sdm, s->chroma_intra_matrix, (2*8*8));
    memcpy(s->chroma_inter_matrix_sdm, s->chroma_inter_matrix, (2*8*8));
        ...
        if (start_code < 0){
            if(s2->pict_type != B_TYPE || avctx->hurry_up==0){
                ...
                if (slice_end(avctx, picture)) {
                    #ifdef MPEG2_ISO_COMPLIANCE
                      if(s2->last_picture_ptr || s2->pict_type == B_TYPE || s2->low_delay)
                    #else
