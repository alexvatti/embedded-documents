...
#ifdef ARCH_ARC
// Temporary workround for MJPEG
extern int suppress_aurora_init;
#endif
...
static void gmc1_c(uint8_t *dst, uint8_t *src, int stride, int h, int x16, int y16, int rounder)
{
	...
	dst_stride = 32;
	dst = dst + (unsigned int)SDM_Bytes ;
    #endif
    for(i=0; i<h; i++)
    {
        ...
        for(x=0; x<8; x++){ //XXX FIXME optimize
            ...
            if((unsigned)src_x < width){
                if((unsigned)src_y < height){
                    ...
                    dst[y*dst_stride + x]= (  (  src[index         ]*(s-frac_x)
                ...
                if((unsigned)src_y < height){
                    ...
                    dst[y*dst_stride + x]= (  (  src[index         ]*(s-frac_y) 
                                           + src[index+stride  ]*   frac_y )*s
                                        + r)>>(shift*2);
                }else{
                    index= clip(src_x, 0, width) + clip(src_y, 0, height)*stride;                    
                    dst[y*dst_stride + x]=    src[index         ];
                }
            }
...
static int rd8x8_c(/*MpegEncContext*/ void *c, uint8_t *src1, uint8_t *src2, int stride, int h){
    ...
    uint64_t __align8 aligned_temp[sizeof(DCTELEM)*64/8];
	#ifdef __MPEG_HCAC__
    	uint64_t __align8 *aligned_bak = (uint64_t *)malloc(stride * sizeof(uint64_t));
	#else
...
void dsputil_init(DSPContext* c, AVCodecContext *avctx)
{
    ...
    if(avctx->lowres==1){
        //if(avctx->idct_algo==FF_IDCT_INT || avctx->idct_algo==FF_IDCT_AUTO){
            ...
            c->idct_put= NULL;  /* Set in ARC specific */
            c->idct_add= NULL;  /* code.               */
        //}else{
            /* Not required in libarcavcodec. */
...
#define SET_CMP_FUNC(name) \
    ...
    if (! suppress_aurora_init) {
#ifdef AURORA_MPEG_MACROS
    aurora_mpeg_record_macros();
#endif

    dsputil_init_aurora(c,avctx);

#ifdef AURORA_QPEL
    dsputil_qpel_init( c);
    dsputil_subpel_init( c );
#endif
    }
