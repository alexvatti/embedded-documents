...
/* ESCAPE_LEVEL is the value of level which encodes an escape code.
 * This must agree with the coding of how VLC hw tables are constructed
 */
#ifndef ARC_BITSTREAM_READER
#define ESCAPE_LEVEL 0
#else
#define ESCAPE_LEVEL 63
#endif
...
static void h263_decode_init_vlc_arc(MpegEncContext *s);
...
#elif  CORRECT_DIRECT_MODE_MV
            s->mv[0][i][0] = s->p_field_mv_table[i][0][mb_index][0]*time_pb/time_pp + mx;
            s->mv[0][i][1] = (2*s->p_field_mv_table[i][0][mb_index][1]*time_pb)/time_pp + my;
            s->mv[1][i][0] = mx ? s->mv[0][i][0] - s->p_field_mv_table[i][0][mb_index][0]
                                : s->p_field_mv_table[i][0][mb_index][0]*(time_pb - time_pp)/time_pp;
            s->mv[1][i][1] = my ? s->mv[0][i][1] - (2*s->p_field_mv_table[i][0][mb_index][1])
                                : (2*s->p_field_mv_table[i][0][mb_index][1]*(time_pb - time_pp))/time_pp;
            s->mv[0][i][1] = s->mv[0][i][1] / 2;
            s->mv[1][i][1] = s->mv[1][i][1] / 2;
#else
            s->mv[0][i][0] = s->p_field_mv_table[i][0][mb_index][0]*time_pb/time_pp + mx;
            s->mv[0][i][1] = s->p_field_mv_table[i][0][mb_index][1]*time_pb/time_pp + my;
            s->mv[1][i][0] = mx ? s->mv[0][i][0] - s->p_field_mv_table[i][0][mb_index][0]
...
#if  defined(CORRECT_DIRECT_MODE_MV)||defined(MPEG4_ISO_COMPLIANCE)
        // set the reference fields for forward/backward prediction
        {
            int xy = s->block_index[0];
            int wrap = s->b8_stride;
            //s->field_select[dir][fld]
            // last picture
            s->field_select[0][0] = s->next_picture.ref_index[0][xy];
            s->field_select[0][1] = s->next_picture.ref_index[0][xy + wrap];
            //next picture
            s->field_select[1][0] = 0;
            s->field_select[1][1] = 1;
        }
#endif
...
//    if(s->pict_type==B_TYPE && !s->readable) return;
    ...
    */
    ...
    //invalidating and flushing cache after deblocking
    _sr(0x1,0x47);
}
...
static int h263_pred_dc(MpegEncContext * s, int n, uint16_t **dc_val_ptr)
{
    ...
    *dc_val_ptr = (uint16_t *)&dc_val[x + y * wrap];
    return pred_dc;
}
static int clipAC (int x)
{
  int clipped;
    ...
    if (s->ac_pred) {
        ...
        if (s->h263_aic_dir) {
            ...
            if (a != 1024) {
                ac_val -= 16;
                for(i=1;i<8;i++) {
                    int tmp = block[s->dsp.idct_permutation[i<<3]];
                    tmp += ac_val[i];
                    block[s->dsp.idct_permutation[i<<3]] = clipAC(tmp);
                }
                pred_dc = a;
            }
        } else {
            ...
            if (c != 1024) {
                ...
                for(i=1;i<8;i++) {
                    ...
                    int tmp = block[s->dsp.idct_permutation[i   ]];
                    tmp += ac_val[i + 8];
                    block[s->dsp.idct_permutation[i   ]] = clipAC(tmp);
...
*/
    wrap= s->block_wrap[n];
    dc_val = (uint16_t *)s->dc_val[0] + s->block_index[n];
...
/**
 * predicts the dir (for intra macro-blocks).
 * @param n block index (0-3 are luma, 4-5 are chroma)
 * returns the prediction direction. 0:HORIZONTAL 1:VERTICAL
 */
static inline int ff_mpeg4_pred_dir(MpegEncContext * s, int n)
{
    int a, b, c, wrap;
    uint16_t *dc_val;
    int dir;
                   ...
                   int dir)
{
    ...
    int16_t *ac_val, *ac_val1;
    int8_t * const qscale_table= s->current_picture.qscale_table;
    uint8_t *idct_permutation = s->dsp.idct_permutation;
    ...
    if (s->ac_pred) {
        if (dir == 0) {
            ...
            if(s->mb_x==0 || s->qscale == qscale_table[xy] || n==1 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                    block[idct_permutation[i<<3]] += ac_val[i];
                }
            }else{
                /* different qscale, we must rescale */
                for(i=1;i<8;i++) {
                    block[idct_permutation[i<<3]] += ROUNDED_DIV(ac_val[i]*qscale_table[xy], s->qscale);
                }
            }
        } else {
            const int xy= s->mb_x + s->mb_y*s->mb_stride - s->mb_stride;
            ...
            if(s->mb_y==0 || s->qscale == qscale_table[xy] || n==2 || n==3){
                /* same qscale */
                for(i=1;i<8;i++) {
                    block[idct_permutation[i]] += ac_val[i + 8];
                }
            }else{
                /* different qscale, we must rescale */
                for(i=1;i<8;i++) {
                    block[idct_permutation[i]] += ROUNDED_DIV(ac_val[i + 8]*qscale_table[xy], s->qscale);
                }
            }
        }
    }
    /* left copy */
    for(i=1;i<8;i++)
        ac_val1[i    ] = block[idct_permutation[i<<3]];

    /* top copy */
    for(i=1;i<8;i++)
        ac_val1[8 + i] = block[idct_permutation[i   ]];
...
void h263_decode_init_vlc(MpegEncContext *s)
{
    ...
    h263_decode_init_vlc_arc(s);
}

static RLTable *g_meta_rtl_table[25];
...
static void h263_decode_init_vlc_arc(MpegEncContext *s)
{
	...
	g_meta_rtl_table[ARC_VLC_TABLE_RL_INTRA] = &rl_intra;
	g_meta_rtl_table[ARC_VLC_TABLE_RL_INTER] = &rl_inter;
	g_meta_rtl_table[ARC_VLC_TABLE_RVLC_RL_INTRA] = &rvlc_rl_intra;
	g_meta_rtl_table[ARC_VLC_TABLE_RVLC_RL_INTER] = &rvlc_rl_inter;
	g_meta_rtl_table[ARC_VLC_TABLE_RL_INTRA_AIC] = &rl_intra_aic;
    }
#endif
}
...
static inline int mpeg4_is_resync(MpegEncContext *s){
    ...
    if(bits_count + 8 >= s->gb.size_in_bits){
        ...
        if(show_bits(&s->gb, 16) == ff_mpeg4_resync_prefix[bits_count&7]){
            int len;
	    BS_CONTEXT_TYPE gb = SAVE_BS_POS(s->gb);
...
static int mpeg4_decode_video_packet_header(MpegEncContext *s)
{
    ...
    if(len!=ff_mpeg4_get_video_packet_prefix_length(s)){
        av_log(s->avctx, AV_LOG_ERROR, "marker does not match f_code len=%d ff_mpeg4_get_video_packet_prefix_length(s)=%d\n",
...
int ff_h263_resync(MpegEncContext *s){
    ...
    RESTORE_BS_POS(s->gb, s->last_resync_gb);
    align_get_bits(&s->gb);
    left= s->gb.size_in_bits - get_bits_count(&s->gb);

    for(;left>16+1+5+5; left-=8){
        if(show_bits(&s->gb, 16)==0){
	    BS_CONTEXT_TYPE bak = SAVE_BS_POS(s->gb);
...
static inline int get_amv(MpegEncContext *s, int n){
    ...
    if(s->real_sprite_warping_points==1)
    {
        ...
        {
#if defined(MPEG4_ISO_COMPLIANCE)||defined(CORRECT_GMC_1_WRAP_PT)
            /* s->sprite_warping_accuracy
            00: 1/2 pel (1-bit for binary precision)
            01: 1/4 pel (2-bit for binary precision)
            10: 1/8 pel (3-bit for binary precision)
            11: 1/16 pel(4-bit for binary precision)
            */
            sum = s->sprite_offset[0][n]<<s->quarter_sample;
    ...
    for(; s->mb_y<s->mb_height; s->mb_y++){
        ...
        for(; s->mb_x<s->mb_width; s->mb_x++){
            ...
            if(s->pict_type==I_TYPE){
                ...
                AURORA_SET_TABLE(ARC_VLC_TABLE_INTRA_MCBPC);
                do{
                ...
                if(s->first_mb_in_slice)
                {
                    s->prev_qscale = s->qscale;
                    s->first_mb_in_slice = 0;
                }
                else
                {
                    s->prev_qscale = s->current_picture.qscale_table[xy - 1];
                }
#endif
                for(i=0; i<6; i++){
                    int dc_pred_dir;
#if defined(MPEG4_ISO_COMPLIANCE)||defined(USE_INTRA_DC_FIX)
                    if(s->prev_qscale < s->intra_dc_threshold)
#endif
                    {
                    ...
                    {
                        dc_pred_dir = ff_mpeg4_pred_dir( s, i);
                    }
#endif
                    dir<<=1;
...
try_again:
...
#else
                AURORA_SET_TABLE(ARC_VLC_TABLE_INTER_MCBPC);
                cbpc = aurora_get_vlc(&s->gb);
#endif
                if (cbpc < 0){
                    av_log(s->avctx, AV_LOG_ERROR, "cbpc corrupted at %d %d\n", s->mb_x, s->mb_y);
                ...
                if(s->mb_intra){
                    ...
                    if ((cbpc & 16) == 0) {
                        ...
                        h263_pred_motion(s, 0, 0, &pred_x, &pred_y);
                        if(!s->mcsel){
                            AURORA_SET_TABLE(ARC_VLC_TABLE_MV);
...
#else
                AURORA_SET_TABLE(ARC_VLC_TABLE_CBPY);
                int cbpy = aurora_get_vlc(&s->gb);
#endif
                if(cbpy<0){
...
//            s->mcsel= 0; //FIXME do we need to init that
    ...
    if (!IS_SKIP(mb_type))
    {
        ...
        if(s->first_mb_in_slice)
        {
            s->prev_qscale = s->current_picture.qscale_table[xy];
            s->first_mb_in_slice = 0;
        }
        else
        {
...
//        const int mv_xy= s->mb_x + 1 + s->mb_y * s->mb_stride;
        ...
        AURORA_SET_TABLE(ARC_VLC_TABLE_H263_MBTYPE_B);
        do{
#ifndef ARC_BITSTREAM_READER
            mb_type= get_vlc2(&s->gb, h263_mbtype_b_vlc.table, H263_MBTYPE_B_VLC_BITS, 2);
#else
            mb_type = aurora_get_vlc(&s->gb);
#endif
            if (mb_type < 0){
...
#else
            AURORA_SET_TABLE(ARC_VLC_TABLE_CBPC_B);
            cbpc = aurora_get_vlc(&s->gb);
#endif
            if(s->mb_intra){
...
#else
            AURORA_SET_TABLE(ARC_VLC_TABLE_MB_TYPE_B);
            mb_type = aurora_get_vlc(&s->gb);
#endif
            if(mb_type<0){
...
intra:
        ...
        if(s->first_mb_in_slice)
        {
            s->prev_qscale = s->qscale; //set prev_qscale to current qscale
            s->first_mb_in_slice = 0;
        }
#endif
        /* decode each block */
...
#if defined(MPEG4_ISO_COMPLIANCE)||defined(USE_INTRA_DC_FIX)
    //set prev_qscale to current qscale after decoding all the blocks
    s->prev_qscale = s->qscale;
#endif
    ...
    if(s->codec_id==CODEC_ID_MPEG4)
    {
#if defined(MPEG4_ISO_COMPLIANCE)||defined(DETECT_STUFFING_BITS)
        if(s->workaround_bugs&FF_BUG_STUFFING_BITS)
        {
            // do not detect stuffing bits for DIVX/XVID
        }
        else
        {
            const int bits_count= get_bits_count(&s->gb);
...
#else
    // AURORA_SET_TABLE(ARC_VLC_TABLE_MV) moved before every call of
    // h263_decode_motion
    // AURORA_SET_TABLE(ARC_VLC_TABLE_MV);
    code = aurora_get_vlc(&s->gb);
#endif
...
#if defined(DEBUG) && !defined(__ARC_STANDALONE_)
...
retry:
    ...
    for(;;)
    {
        ...
        AURORA_GET_RL_VLC(level, run, &s->gb);
#endif
        ...
        else
        {
            ...
            last = code >= rl->last;
#else
            /* run and level have already been extracted directly from hw */
            run--; // XXX because encoded value in hw table already added 1
            if (run >= 192)
            {
                last = 1;
                run -= 192;
        ...
        if (i >= 64)
        {
            ...
            if(s->alt_inter_vlc && rl == &rl_inter && !s->mb_intra){
                ...
                rl = &rl_intra_aic;
#else
            if(s->alt_inter_vlc && rl == ARC_VLC_TABLE_RL_INTER && !s->mb_intra){
                rl = ARC_VLC_TABLE_RL_INTRA_AIC;
#endif
                i = 0;
                RESTORE_BS_POS(s->gb, gb);
        ...
        j = scan_table[i];
        if (s->mb_intra && s->h263_aic && j != 0) {
          int sign = level >= 0 ? +1 :0 ;
          if(!sign)
          {
            level = level * -1;
          }
          block[j] = (sign ? qscale * (2 * level) : -(qscale * (2 * level)));
        }
        else
        {
          block[j] = level;
...
#else
    AURORA_SET_TABLE((n < 4) ? ARC_VLC_TABLE_MP4_DC_LUM : ARC_VLC_TABLE_MP4_DC_CHROM);
    code = aurora_get_vlc(&s->gb);
#endif
    ...
    if(intra) {
      ...
      if(s->qscale < s->intra_dc_threshold){
        ...
        i = 0;
            ...
            i = -1;
            dc_pred_dir = ff_mpeg4_pred_dir(s, n);
      }
        if (!coded)
            goto not_coded;
...
#else
        rl_vlc = (rvlc) ? ARC_VLC_TABLE_RVLC_RL_INTRA : ARC_VLC_TABLE_RL_INTRA;
#endif
        if (s->ac_pred)
        {
            if (dc_pred_dir == 0)
...
#else
        rl_vlc = (rvlc) ? ARC_VLC_TABLE_RVLC_RL_INTER : ARC_VLC_TABLE_RL_INTER;
#endif
        scan_table = s->intra_scantable.permutated;
...
#else
            need_reverse_quantize = 1;
            rl_vlc = (rvlc) ? ARC_VLC_TABLE_RVLC_RL_INTER : ARC_VLC_TABLE_RL_INTER;
#endif
        }
#endif	//!defined(ARC_BITSTREAM_READER)||!defined(SIMD_MPEG4_DEQUANT)
    }

  {
    AURORA_SET_TABLE(rl_vlc);
    OPEN_READER(re, &s->gb);
    for(;;)
    {
        UPDATE_CACHE(re, &s->gb);
...
#endif	//#if	!defined(ARC_BITSTREAM_READER)||!defined(SIMD_MPEG4_DEQUANT)
                    i+= run + 1;
                    if(last) i+=192;
                }
                else
...
#if	defined(ARC_BITSTREAM_READER)&&defined(SIMD_MPEG4_DEQUANT)
                    AURORA_GET_RL_VLC(level, run, &s->gb);
                    i+= run + g_meta_rtl_table[rl_vlc]->max_run[run>>7][level] +1;
#elif defined(ARC_BITSTREAM_READER)
                    AURORA_GET_RL_VLC(level, run, &s->gb);
                    if (need_reverse_quantize)
	                    level = level * qmul + qadd;
                    i+= run + g_meta_rtl_table[rl_vlc]->max_run[run>>7][level/qmul] +1;
#else
                    GET_RL_VLC(level, run, re, &s->gb, rl_vlc, TEX_VLC_BITS, 2);
...
#if	defined(ARC_BITSTREAM_READER)&&defined(SIMD_MPEG4_DEQUANT)
                AURORA_GET_RL_VLC(level, run, &s->gb);
                level = level + g_meta_rtl_table[rl_vlc]->max_level[run>>7][(run-1)&63];//FIXME opt indexing
#elif defined(ARC_BITSTREAM_READER)
                AURORA_GET_RL_VLC(level, run, &s->gb);
                if (need_reverse_quantize)
                    level = level * qmul + qadd;
                level = level + g_meta_rtl_table[rl_vlc]->max_level[run>>7][(run-1)&63] * qmul;//FIXME opt indexing
#else
                GET_RL_VLC(level, run, re, &s->gb, rl_vlc, TEX_VLC_BITS, 2);
...
static void mpeg4_decode_sprite_trajectory(MpegEncContext * s, GetBitContext *gb)
{
    ...
    AURORA_SET_TABLE(ARC_VLC_TABLE_SPRITE);
...
#if defined(MPEG4_ISO_COMPLIANCE)||defined(CORRECT_GMC_1_WRAP_PT)
    if(s->num_sprite_warping_points < 2)
#else
...
//                printf("width/height: %d %d\n", width, height);
        ...
        if((s->mpeg_quant=get_bits1(gb))){ /* vol_quant_type */
            ...
            for(i=0; i<64; i++){
                ...
                s->intra_matrix_sdm[j]= v;

                v= ff_mpeg4_default_non_intra_matrix[i];
                s->inter_matrix[j]= v;
                s->chroma_inter_matrix[j]= v;
                s->inter_matrix_sdm[j]= v;
            }
        ...
        if (s->scalability) {
	    BS_CONTEXT_TYPE bak = SAVE_BS_POS(*gb);
            int ref_layer_id;
