...
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic){
    ...
    if(buf->base[0]){
        ...
        for(i=0; i<3; i++){
            ...
            buf->base[i]= av_malloc((buf->linesize[i]*h>>v_shift)+512);
            if(buf->base[i]==NULL) return -1;
				#ifdef ARCH_ARC
				uint8_t *ptr = (uint8_t *)(((unsigned int)buf->base[i] + 256) & -256);
				#endif
            ...
            if(s->flags&CODEC_FLAG_EMU_EDGE)
            	...
            	{
					#ifdef 	ARCH_ARC
						buf->data[i] = ptr + ALIGN((buf->linesize[i]*EDGE_WIDTH>>v_shift) + (EDGE_WIDTH>>h_shift), STRIDE_ALIGN);
					#else
