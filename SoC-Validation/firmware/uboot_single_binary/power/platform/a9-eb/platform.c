...
struct appf_cluster aem_cluster[] __attribute__ ((section (".APPF_TABLE_DATA")))
    = {{CPU_A9MPCORE, 0x0200, 0, 0, 0xD46F2000, 0xD46F3000, 0xD46F4000, (void *)0, (void *)0, 0}};
...
int appf_platform_boottime_init(void)
{
  ...
  //  *(unsigned *)0x20 = (unsigned)&platform_reset_handler;
   ...
   // }
    ...
    aem_cluster[0].context->lock                 = (void *)appf_device_memory;
    ...
    for (i=0; i<aem_cluster[0].num_cpus; ++i)
    {
        ...
        platform_cpu_stacks[i] = get_memory(STACK_SIZE) + STACK_SIZE;
    }
 ...
 */
appf_platform_late_init(struct appf_cluster *cluster)
{
    /*
