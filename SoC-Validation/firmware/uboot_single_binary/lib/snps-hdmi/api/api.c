...
static state_t api_mCurrentState = API_HPD;
static u8 api_mHpd = FALSE;
static mutex_t api_mMutex = ~0; /* NULL is 0, and kernel returns 0 for first instance of mutex */
...
u8 api_ReadConfig()
{
	...
	for (addr = 0; addr < 0x8000; addr++)
	{
		...
		(addr >= 0x0100 && addr <= 0x0109) || /* IH */
		(addr >= 0x0180 && addr <= 0x0189) || /* IH */
		...
		{
#ifdef CONFIG_SHIKHARA 
                        LOG_WRITE(addr, access_CoreReadByte(addr + api_mBaseAddress));
#else
