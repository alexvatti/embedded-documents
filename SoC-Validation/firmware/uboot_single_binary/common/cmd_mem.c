...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (size == 5) {
	   if(addr > 0x3FF ){
	   	printf("NVM address range is 0x00 - 0x3FF\n Exiting...");
	   	return -1; 
	   }
	   else{
		char nvm_buff[length+2];
		nvm_read_buff(nvm_buff, addr, length);
		print_buffer(addr, (void*)nvm_buff, 4, length, DISP_LINE_LEN/4);
	   	return rc;
	    }
	}
#endif
