...
#if defined(CONFIG_DSI_TRULY)
static int do_dsi_bmp_display(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[]);
#endif
static int do_lcd_bmp_display(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[]);
...
#if defined(CONFIG_DSI_TRULY)
	U_BOOT_CMD_MKENT(dsi_display, 5, 0, do_dsi_bmp_display, "", ""),
#endif
	U_BOOT_CMD_MKENT(lcd_display, 5, 0, do_lcd_bmp_display, "", ""),
};
...
#if defined(CONFIG_DSI_TRULY)
	"bmp mipi_display <imageAddr> [x y] - dsi display image at x,y"
#endif
	"bmp lcd_display <imageAddr> [x y] - lcd display image at x,y"
);
...
static int do_dsi_bmp_display(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	ulong addr,x,y;
        ...
        dsi_display_bitmap_custom((ulong)bmp, x, y);
...
static int do_lcd_bmp_display(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	lcd_display_bitmap_custom((ulong)bmp, x, y);
