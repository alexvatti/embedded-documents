...
int main(int argc, char **argv)
{
	...
	//ret = usb_bulk_write(devh, 3, (void *)dl_data, dl_size, 5*1000*1000);
     /* Generic bulk write function provided by smdk utitlity */	
     #if 0
	ret = usb_bulk_write(devh, ep_out, (void *)dl_data, dl_size, 5*1000*1000);
	printf("=> usb_bulk_write() returned %d\n", ret);
     /* Modified bulk write giving 2k bulk data each time */
     #else
       {
         unsigned int bytes_per_turn = 2048;
         unsigned int bytes_remaining = dl_size;
         unsigned char * start_urb_submit = dl_data;
         unsigned int i, j;
         for (i =0; i < dl_size/bytes_per_turn; i++)
	 {

           ret += usb_bulk_write(devh, ep_out, (void *)start_urb_submit, bytes_per_turn, 5*1000*1000);
           for(j=0;j<200000;j++);
           bytes_remaining -=bytes_per_turn;
           start_urb_submit +=bytes_per_turn; 
           
	 }
         if(bytes_remaining)
         {
            ret += usb_bulk_write(devh, ep_out, (void *)start_urb_submit, bytes_remaining, 5*1000*1000);
