...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	for (cur_ep = 0; cur_ep < num_of_ep; cur_ep++) {
		...
		virt_dev->eps[ep_index].ring = xhci_ring_alloc(1, 1);
	...
	return xhci_configure_endpoints(udev, 0);
}
...
int xhci_check_maxpacket(struct usb_device *udev)
{
	...
	if (hw_max_packet_size != max_packet_size) {
		...
		ret = xhci_configure_endpoints(udev, 1);
	}
	return ret;
}
			...
			void *buffer, struct devrequest *req)
{
	...
	case USB_REQ_GET_STATUS | ((USB_RT_PORT | USB_DIR_IN) << 8):
		debug("USB_REQ_GET_STATUS | ((USB_RT_PORT | USB_DIR_IN)\n");
		memset(tmpbuf, 0, 4);
		reg = xhci_readl(status_reg);
		debug("Status register 0x%x\n",reg);
		if (reg & PORT_CONNECT) {
	...
	case USB_REQ_CLEAR_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		debug("USB_REQ_CLEAR_FEATURE | ((USB_DIR_OUT | USB_RT_PORT)\n");
		reg = xhci_readl(status_reg);
		reg = xhci_port_state_to_neutral(reg);
		switch (le16_to_cpu(req->value)) {
		case USB_PORT_FEAT_ENABLE:
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	debug("****HCCPARAMS register value = 0x%x \n",reg);
